...
class RCache {
     ...
     */
    async isGroupExists(groupKey, ) {
        const v = await this.client.exists(groupKey);
        return !!v;
    }
    
    /**
     * check if a key exists in a group
     * @param {*} groupKey 
     * @param {*} entityKeyObj 
     * @returns 
     */
    async isKeyExists(groupKey, entityKeyObj) {
        const key = this.getKey(entityKeyObj);
        return this.client.hExists(groupKey, key);
    }
